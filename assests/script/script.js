...
window.addEventListener("scroll", function () {
    ...
    const logos = document.querySelectorAll("header .nav-bar h2 a");

    if (!header) return;

    if (window.scrollY > 0) {
        header.classList.add("scrolled");
        logos.forEach((logo) => logo.classList.add("colour-effect"));
        links.forEach((link) => link.classList.add("colour-effect"));
    } else {
        header.classList.remove("scrolled");
        logos.forEach((logo) => logo.classList.remove("colour-effect"));
        links.forEach((link) => link.classList.remove("colour-effect"));
    }
});
...
const cardLinks = {
    ...
    "ai-ml": "assests/pages/ai-ml.html",
    "video-editing": "assests/pages/video-editing.html",
};
